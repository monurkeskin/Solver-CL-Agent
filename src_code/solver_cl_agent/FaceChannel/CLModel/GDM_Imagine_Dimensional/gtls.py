...
def plot_network(net, edges, labels, label):
    ...
    for ni in range(len(net.weights)):
        ...
        if labels:
            ...
            if dim_net:
                plt.scatter(net.weights[ni][0], net.weights[ni][1], alpha=0.5)
            else:
                plt.scatter(net.weights[ni][0, 0], net.weights[ni][0, 1], alpha=0.5)
        if edges:
            for nj in range(len(net.weights)):
                if net.edges[ni, nj] > 0:
                    if dim_net:
                        plt.plot(
                            [net.weights[ni][0], net.weights[nj][0]],
                        ...
                        plt.plot(
                            [net.weights[ni][0, 0], net.weights[nj][0, 0]],
