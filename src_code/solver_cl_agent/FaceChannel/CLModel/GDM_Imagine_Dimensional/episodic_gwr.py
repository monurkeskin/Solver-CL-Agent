...
class EpisodicGWR(GammaGWR):
    ...
    def init_network(
        self, ds, labels, num_context, max_nodes, age=600, av_delta=0.1, random=False
    ):
    ...
    def update_labels(self, bmu, label, **kwargs):
        ...
        if not new_node:
            ...
            self.alabelsArousal = np.concatenate(
                (self.alabelsArousal, [label[1]]), axis=0
            )
            self.alabelsValence = np.concatenate(
                (self.alabelsValence, [label[0]]), axis=0
            )
            ...
            for iteration in range(0, self.samples):
                ...
                if (not self.regulated) or (self.regulated and misclassified):
                    ...
                    if (
                        ...
                        super(EpisodicGWR, self).update_edges(
                            b_index, s_index, new_index=n_index
                        )
                        ...
                        # Habituation counter
                        super(EpisodicGWR, self).habituate_node(
                            n_index, self.tau_b, new_node=True
                        )
    ...
    def test_av(self, ds_vectors, ds_labels, test_accuracy=False):
        ...
        if test_accuracy:
            ...
            ccc_v = np.round(
                concordance_correlation_coefficient(ds_labels[:, 0], valences), 2
            )
            ccc_a = np.round(
                concordance_correlation_coefficient(ds_labels[:, 1], arousals), 2
            )
            ...
            return (
                ...
                np.hstack(
                    [
                        np.array(valences).reshape((len(valences), 1)),
                        np.array(arousals).reshape((len(arousals), 1)),
                    ]
                ),
            )
        return self.bmus_weight, np.hstack(
            [
                np.array(valences).reshape((len(valences), 1)),
                np.array(arousals).reshape((len(arousals), 1)),
            ]
