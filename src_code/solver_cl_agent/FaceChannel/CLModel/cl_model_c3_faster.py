    ...
    with graph.as_default():
        ...
        if mode == "imagine":
            ...
            for file in files:
                ...
                encodings = np.array(
                    dense_model.predict(
                        np.array(processed_faces),
                        batch_size=modelDimensional.BATCH_SIZE,
                    )
                ).reshape((49, 200))
        ...
        # ! ENCODE MODE.
            ...
            dense_model = modelDimensional.get_dense_model()
            encodings = dense_model.predict(np.array(processed_faces))
        ...
        # ! INPUT MODE.
            ...
            dimensions = modelDimensional.model.predict(
                np.array(processed_faces), batch_size=modelDimensional.BATCH_SIZE
            )

            arousal = np.array([dimension for dimension in dimensions[0]]).reshape(
                (len(dimensions[0]), 1)
            )
            valence = np.array([dimension for dimension in dimensions[1]]).reshape(
                (len(dimensions[1]), 1)
            )
            dense_model = modelDimensional.get_dense_model()
            encodings = dense_model.predict(
                np.array(processed_faces), batch_size=modelDimensional.BATCH_SIZE
            )
...
def get_arousal_valence(
    log_dir: str, gwrs_path: str, graph, sess, use_imagined: bool = True
):
...
def predict(log_dir: str, gwrs_path: str, graph, sess):
    if len(os.listdir(gwrs_path)) > 0:
        input_trained_GWRs = []
        input_trained_GWRs.append(
            gtls.import_network(file_name=gwrs_path + "GDM_E", NetworkClass=EpisodicGWR)
    ...
    output_trained_GWRs = input_trained_GWRs
