#!/usr/bin/env python
"""
CLIFER Continual Learning Model (cl_model_c3)

This module implements the core CLIFER (Continual Learning with Imagination for
Facial Expression Recognition) algorithm. It combines:

1. **CAAE Generator**: Conditional Adversarial Autoencoder for generating
   imagined facial expressions at different arousal/valence levels
   
2. **GDM-E (Episodic Memory)**: Fast-adapting Growing When Required network
   for rapid personalization to user-specific expressions
   
3. **GDM-S (Semantic Memory)**: Slow-consolidating GWR network for stable
   long-term affect representation

The main entry point is `get_arousal_valence()` which runs the full pipeline.

References:
    - Paper Section 4.1: Generative Replay
    - Algorithm 1: Asynchronous Solver-CL
"""
...
def tile_to_square(images):
    ...
    for index, image in enumerate(images):
        ...
        index_row = index // 7
        frame[
            (index_row * 96) : ((index_row + 1) * 96),
            (index_column * 96) : ((index_column + 1) * 96),
        ] = image
    return frame
...
def generate_images(
    ...
    sess,
    graph,
    path_to_dir="./logs/faces/",
    path_to_out_dir="./logs/faces_imagined/",
):
    with graph.as_default():
        ...
        for file in files:
            i = load_image_as_network_input(path_to_dir + "/" + file).reshape(
                (1, 96, 96, 3)
            )
            ...
            feed_dict = {
                arousal_tensor: arousal,
                valence_tensor: valence,
                images_tensor: query_images,
            }
            ...
            save_generated_output(
                i, x, path_to_out_dir + "/" + file, valence=valence, arousal=arousal
            )
...
def generate_encodings_facechannel(path_to_dir, sess, graph, mode="imagine"):
    ...
    with graph.as_default():
        ...
        modelDimensional = modelLoader.modelLoader(
            modelDictionary.DimensionalModel, printSummary=False
        )
        # ! IMAGINE MODE.
        if mode == "imagine":
            ...
            for file in files:
                ...
                for img in imgs:
                    ...
                    processedFace = preprocess(
                        load_image_as_network_input(
                            path_to_dir + file + "/" + img,
                    ...
                    # get encodings
                    encodings.append(
                        numpy.array(modelDimensional.getDense(processedFace))
                    )
        ...
        # ! ENCODE MODE.
            ...
            if len(files) < 90:
                ...
                files = numpy.concatenate(
                    [
                        files,
                        [
                            files[i]
                            for i in numpy.random.randint(0, len(files), to_fill)
                        ],
                    ]
                )
            else:
            ...
            face_time_stamps = [
                facename.split("_frame_")[1].split(".jpg")[0] for facename in files
            ]
            encodings = []
            for file in files:
                processedFace = preprocess(
                    load_image_as_network_input(
                        path_to_dir + file, facechannel_faceSize, normalise=False
                    ).reshape((64, 64, 3)),
                    facechannel_faceSize,
                )
                # get encodings
...
def generate_encodings(loadPath, path_to_out_dir, sess, graph, test=False):
    ...
    with sess:
        ...
        if not test:
            ...
            for file in files:
                query_images.append(
                    load_image_as_network_input(path_to_out_dir + file).reshape(
                        (1, 96, 96, 3)
                    )
                )
            query_images = np.array(query_images).reshape(
                (len(query_images), 96, 96, 3)
            )
...
def apply_network_to_images_of_dir(
    loadPath,
    sess,
    graph,
    path_to_dir="./logs/faces/",
    ...
    arousal = np.asarray([item for sublist in arousal for item in sublist]).reshape(
        (49, 1)
    )

    generate_images(
        loadPath, valence, arousal, sess, graph, path_to_dir, path_to_out_dir
    )

    return generate_encodings_facechannel(
        path_to_dir=path_to_out_dir, mode="imagine", sess=sess, graph=graph
    )
...
def train_GDM(data, labels, trained_GWRs):
    ...
    if trained_GWRs[0] is None:
        ...
        g_episodic.init_network(
            data, ds_labels, num_context, max_nodes=len(data), age=600
        )
        ...
        g_semantic.init_network(
            data, ds_labels, num_context, max_nodes=len(data) // 2, age=1200
        )
    else:
    ...
    for s in range(0, ds_vectors.shape[0], batch_size):
        ...
        e_weights, e_labels = g_episodic.test_av(
            ds_vectors[s : (s + batch_size)],
            ds_labels[s : (s + batch_size)],
            test_accuracy=False,
        )
...
def annotate_GDM(trained_GWRs, test_data):
    ...
    return numpy.array([e_arousals, e_valences]).reshape(
        (len(e_arousals)), 2
...
def get_arousal_valence(
    ...
    """
    Main CLIFER pipeline: personalized arousal-valence prediction.
    
    This is the primary entry point for the continual learning affect model.
    It performs the following steps:
    
    1. Load or initialize GDM-E and GDM-S memory networks
    2. Generate imagined faces using CAAE (49 emotion variants per real face)
    3. Encode all faces using FaceChannel's dense layer
    4. Train/update dual-memory GWR networks with combined real + imagined data
    5. Annotate current session frames with personalized arousal/valence
    6. Save updated GWR networks for next round
    
    Args:
        log_dir: Path to session log directory containing:
                 - faces/: captured face frames
                 - imagined/: generated faces (created if missing)
                 - saved_GWRs/: persisted memory networks
        graph: TensorFlow graph for CAAE model
        sess: TensorFlow session
        use_imagined: If True, generate and use imagined samples (default: True)
                      Set to False for ablation studies
    
    Returns:
        List of (arousal, valence) predictions for each frame,
        each shaped as numpy array (2, 1, 1)
    
    Note:
        This function modifies the saved_GWRs/ directory, updating
        GDM_E and GDM_S network files after each call.
    """
    ...
    # Imagining Faces
    if use_imagined:
        imagined_data, imagined_labels = apply_network_to_images_of_dir(
            loadPath=CAAE_LoadPath,
            sess=sess,
            graph=graph,
    ...
    # Encode Input data
    input_data, input_labels = generate_encodings_facechannel(
        path_to_dir=log_dir + "faces/", sess=sess, graph=graph, mode="input"
    )
    ...
    output_trained_GWRs = train_GDM(
        data=training_data, labels=training_labels, trained_GWRs=input_trained_GWRs
    )

    encodings, face_time_stamps = generate_encodings_facechannel(
        path_to_dir=log_dir + "faces/", sess=sess, graph=graph, mode="encode"
    )
