...
class AssociativeGWR:
    ...
    def habituateNeuron(self, index, tau):
        self.habn[index] += tau * 1.05 * (1.0 - self.habn[index]) - tau

    def updateNeuralWeight(self, input, index, epsilon):
        delta = (
            np.array([np.dot((input - self.weights[index]), epsilon)])
            * self.habn[index]
        )
    ...
    def trainAGWR(self, dataSet, mE, iT, eeB, eeN):
        ...
        while epochs < self.maxEpochs:
            ...
            for iteration in range(0, self.samples):
                ...
                for i in range(0, self.numNodes):
                    distances[i] = self.computeDistance(
                        self.weights[i], input, self.distanceMetric
                    )
                ...
                if (
                    (a < self.insertionThreshold)
                    and (self.habn[firstIndex] < self.habThreshold)
                    and (self.numNodes < self.maxNodes)
                ):
                    # Add new neuron
                    newWeight = np.array(
                        [np.dot(self.weights[firstIndex] + input, self.newNodeValue)]
                    )
