...
class FaceChannelV1:
    ...
    def __init__(self, type="Cat", loadModel=True, numberClasses=7):
        ...
        folderName = Path(
            os.path.abspath(sys.modules[FaceChannelV1.__module__].__file__)
        ).parent
        ...
        if type == "Cat":
            modelDirectory = (
                folderName / "TrainedNetworks" / "CategoricalFaceChannel.h5"
            )
        elif type == "Dim":
            modelDirectory = (
                folderName / "TrainedNetworks" / "DimensionalFaceChannel.h5"
            )
        else:
    ...
    def predict(self, images, preprocess=True):
        ...
        if preprocess:
            ...
            for img in images:
                ...
                if (
                    not len(face) == 0 and not facePoints == None
                ):  # If a face is detected
                    face = self.imageProcessing.preProcess(
                        face, imageSize=self.IMAGE_SIZE
        ...
        classification = self.model.predict(
            images, batch_size=self.BATCH_SIZE, verbose=0
        )
    ...
    def getCategoricalModel(self, numberClasses):
        ...
        categoricalOutput = Dense(
            units=numberClasses, activation="tanh", name="categoricalOutput"
        )(dense)
