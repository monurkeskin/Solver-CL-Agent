...
from Agent_Strategy.TSBT import TSBT
from Agent_Emotion.emotion_controller import EmotionController
...
class TSBTAgent:
    ...
    def receive_offer(self, human_offer):
        ...
        if self.utility_space.get_offer_utility(
            ...
            return negoAction.Accept("Agent"), "accept"
        else:
            self.opponent_previous_offer_utility = self.utility_space.get_offer_utility(
                human_bid
            )
            return negoAction.Offer(generated_offer), emotion
