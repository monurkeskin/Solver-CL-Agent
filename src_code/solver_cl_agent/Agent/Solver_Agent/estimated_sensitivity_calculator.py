...
class EstimatedSensitivityCalculator:
    ...
    def get_opponent_moves_list(
        ...
        for offer in targetHistory:
            ...
            offer_to_opponent = [4 - value for value in offer.get_bid()]
        ...
        for i in range(len(targetHistory) - 1):
            ...
            if (
                abs(target_bid_utility_list[i + 1] - target_bid_utility_list[i]) == 0
                and abs(human_bid_utility_list[i + 1] - human_bid_utility_list[i]) == 0
            ):
    ...
    def get_nash_offer(
        self, agent_preference_profile, estimated_opponent_preference, index
    ):
        nash_products = []
        real_nash_products = []
        agent_all_offers = agent_preference_profile.all_possible_offers[:]
        real_opponent_preference = UtilitySpace("HATN/Domain/Fruit/Fruits_A_Human.xml")

        for offer in agent_all_offers:
            offer_to_human = agent_preference_profile.calculate_offer_for_opponent(
                offer
            )
            offer_utility_to_human = estimated_opponent_preference.get_offer_utility(
                offer_to_human
            )
            ...
            nash_products.append(
                ((agent_offer_utility * offer_utility_to_human), offer)
            )
            real_nash_products.append(
                ((agent_offer_utility * real_offer_utility_to_human), offer)
            )

        nash_products.sort(reverse=True, key=lambda x: (x[0], sum(x[1])))
