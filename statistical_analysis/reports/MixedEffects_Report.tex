...
\vspace{0.5em}
\begin{tcolorbox}[colback=green!10,colframe=green!50!black,title=\textbf{4 Robust Significant Effects}]
\begin{center}
\begin{tabular}{lcccl}
\toprule
...
A-V Correlation & $\Delta r = 0.60$ & $< .001$ & FC: $-0.15$ $\rightarrow$ CL: $+0.45$ \\
Agent Utility & $d = 0.31$ & $.017$ & CL $\uparrow$ (+2.2pp) \\
Concession Rate & $d = 0.32$ & $.013$ & CL $\uparrow$ (+33\% relative) \\
\bottomrule
\end{tabular}
\end{center}
\textit{Note: Agreement Rounds no longer significant at $\alpha=0.05$ ($p = .134$, $d = -0.19$)}
\end{tcolorbox}

\vspace{0.5em}
\begin{tcolorbox}[colback=gray!5,colframe=gray!50!black,title=\textbf{Data Sources}]
\begin{tabular}{ll}
\textbf{Affect Analysis}: & \texttt{final\_evaluation\_results.csv} ($N_{rounds} = 2,114$) \\
...
\begin{itemize}
    \item \textbf{Level 1}: Rounds (N$_{FC}$ = 1,134, N$_{CL}$ = 980)
...
\midrule
Affect (all rounds) & 1,134 & 980 & Deduplicated round-level data \\
Affect (Round $> 1$) & 1,068 & 914 & Excludes Round 1 \\
Moves (all valid) & 1,068 & 904 & Round $> 1$, any move type \\
Moves (main 6 types) & 1,046 & 874 & Excludes ``Other'' and rare types \\
\bottomrule
\end{tabular}
...
\midrule
\multicolumn{6}{l}{\textit{Round-Level (N$_{FC}$=1,134, N$_{CL}$=980)}} \\
...
\midrule
\rowcolor{green!15} Arousal & $0.02 \pm 0.07$ & $\mathbf{0.14 \pm 0.16}$ & 5.56 & $<.001$*** & \textbf{0.69} & \textcolor{green!50!black}{\textbf{SIG}} \\
Valence & $0.03 \pm 0.13$ & $0.05 \pm 0.13$ & 1.01 & .317 & 0.13 & n.s. \\
\bottomrule
\end{tabular}
...
\textbf{Arousal:} Highly robust at both levels. Large effect at round level ($d=0.88$), medium-large at subject level ($d=0.69$).\\
...
\midrule
FC (FaceChannel) & $-0.15$ & 1,134 & Round \\
\rowcolor{green!15} CL (Continual Learning) & $\mathbf{0.45}$ & 980 & Round \\
\midrule
...
\textbf{Metric} & \textbf{FC} & \textbf{CL} & \textbf{t(65)} & \textbf{p} & \textbf{d} & \textbf{Status} \\
\midrule
\rowcolor{green!15} Agent Utility & $0.73 \pm 0.08$ & $\mathbf{0.75 \pm 0.06}$ & 2.46 & \textbf{.017}* & \textbf{0.31} & \textcolor{green!50!black}{\textbf{SIG}} \\
User Utility & $0.76 \pm 0.09$ & $0.75 \pm 0.10$ & $-0.69$ & .495 & $-0.09$ & n.s. \\
\rowcolor{orange!15} Agreement Rounds & $8.56 \pm 5.02$ & $7.61 \pm 3.83$ & $-1.52$ & .134 & $-0.19$ & n.s. \\
\bottomrule
\end{tabular}
...
\begin{tcolorbox}[colback=green!5,colframe=green!50!black]
\textbf{Agent Utility:} CL achieves significantly higher utility ($p = .017$, $d = 0.31$).\\
\textbf{Agreement Rounds:} CL trend toward faster agreements ($-0.95$ rounds) but not statistically significant ($p = .134$).
\end{tcolorbox}
...
Move types from \texttt{move\_affect\_comparison.csv} (Round $> 1$, N$_{FC}$=1,068, N$_{CL}$=904):
...
\midrule
Concession & 473 & 44.3\% & 386 & 42.7\% & 1.45 & .229 \\
Selfish & 470 & 44.0\% & 393 & 43.5\% & 0.15 & .699 \\
Fortunate & 50 & 4.7\% & 49 & 5.4\% & 1.53 & .216 \\
Nice & 12 & 1.1\% & 8 & 0.9\% & 0.61 & .434 \\
Unfortunate & 37 & 3.5\% & 30 & 3.3\% & 0.06 & .813 \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{green!15} \textbf{Concession} & $0.28 \pm 0.20$ & $\mathbf{0.37 \pm 0.27}$ & 2.55 & \textbf{.013}* & \textbf{0.32} & \textcolor{green!50!black}{\textbf{SIG}} \\
Nice & $0.04 \pm 0.08$ & $0.04 \pm 0.09$ & 0.23 & .821 & 0.03 & n.s. \\
Fortunate & $0.18 \pm 0.16$ & $0.15 \pm 0.13$ & $-1.11$ & .272 & $-0.14$ & n.s. \\
Unfortunate & $0.22 \pm 0.17$ & $0.21 \pm 0.17$ & $-0.25$ & .803 & $-0.03$ & n.s. \\
Selfish & $0.23 \pm 0.19$ & $0.19 \pm 0.16$ & $-1.38$ & .172 & $-0.17$ & n.s. \\
...
\begin{tcolorbox}[colback=green!5,colframe=green!50!black]
\textbf{Key Finding:} At subject-level, concession rate is significantly higher in CL (37\% vs 28\%, $p = .013$, $d = 0.32$).\\[2mm]
\textbf{Clarification of ``33\% increase'' in paper:} This refers to a \textit{relative} increase: $(0.37 - 0.28) / 0.28 = 33\%$. The \textit{absolute} increase is +9.3 percentage points.
\end{tcolorbox}
...
\midrule
Concession & 473 & 386 & 859 & 44.7\% \\
Selfish & 470 & 393 & 863 & 44.9\% \\
Fortunate & 50 & 49 & 99 & 5.2\% \\
Unfortunate & 37 & 30 & 67 & 3.5\% \\
Nice & 12 & 8 & 20 & 1.0\% \\
Silent & 4 & 8 & 12 & 0.6\% \\
\midrule
\textbf{TOTAL} & \textbf{1,046} & \textbf{874} & \textbf{1,920} & 100\% \\
\bottomrule
\end{tabular}
...
\rowcolor{green!15} Arousal & Subject & $<.001$ & 0.69 & \textcolor{green!50!black}{\textbf{SIG***}} \\
Valence & Round & $<.001$ & 0.18 & Round only \\
\rowcolor{orange!15} Valence & Subject & .317 & 0.13 & n.s. \\
\rowcolor{green!15} A-V Correlation & Round & $<.001$ & -- & \textcolor{green!50!black}{\textbf{SIG***}} \\
\rowcolor{green!15} Agent Utility & Subject & .017 & 0.31 & \textcolor{green!50!black}{\textbf{SIG*}} \\
\rowcolor{orange!15} User Utility & Subject & .495 & $-0.09$ & n.s. \\
\rowcolor{orange!15} Agreement Rounds & Subject & .134 & $-0.19$ & n.s. \\
\rowcolor{green!15} Concession Rate & Subject & .013 & 0.32 & \textcolor{green!50!black}{\textbf{SIG*}} \\
\rowcolor{orange!15} Other Moves & Subject & $>.10$ & $<0.17$ & n.s. \\
\bottomrule
\end{tabular}
\caption{Complete Summary: 4 of 10 Tests Significant at $\alpha = .05$ (*** $p<.001$, * $p<.05$)}
\end{table}
...
\midrule
Arousal (Round $d$) & 0.88 & 0.88 & \checkmark Yes \\
Arousal (Subject $d$) & 0.69 & 0.69 & \checkmark Yes \\
Valence (Round $d$) & 0.18 & 0.18 & \checkmark Yes \\
Agent Utility (p) & .017 & .017 & \checkmark Yes \\
Agent Utility (d) & 0.31 & 0.31 & \checkmark Yes \\
Agreement Rounds (p) & .134 & .134 & \checkmark Yes (n.s.) \\
Concession (p) & .013 & .013 & \checkmark Yes \\
Concession (d) & 0.32 & 0.32 & \checkmark Yes \\
\bottomrule
\end{tabular}
\caption{Paper Statistics Match Verification -- All Values Verified}
\end{table}
...
\begin{enumerate}
    \item \textbf{Arousal perception}: Large effect at round level ($d = 0.88$), medium-large at subject level ($d = 0.69$), 95\% CI [0.08, 0.17], 75.8\% of subjects favor CL
    \item \textbf{Affect coherence}: A-V correlation shifts from $r = -0.15$ (incoherent) to $r = 0.45$ (theoretically expected) at round level
    \item \textbf{Agent Utility}: +2.2pp improvement ($d = 0.31$, $p = .017$), 95\% CI [0.01, 0.04]
    \item \textbf{Reciprocity Effect}: +9.3pp concession increase ($d = 0.32$, $p = .013$), 33\% relative increase
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Agreement Rounds}: Trend toward faster ($-0.95$ rounds) but not significant ($p = .134$)
    \item \textbf{User Utility}: No significant change ($p = .495$)
...
\begin{quote}
\textit{``The CLIFER-enabled agent achieved significantly higher arousal perception accuracy ($d = 0.69$, $p < .001$; 95\% CI [0.08, 0.17]), with 76\% of participants showing improvement. This translated to better negotiation outcomes: +2.2pp agent utility ($p = .017$) and 33\% more participant concessions ($p = .013$). These effects demonstrate that personalized affective perception promotes mutual cooperation in human-agent negotiation.''}
\end{quote}
